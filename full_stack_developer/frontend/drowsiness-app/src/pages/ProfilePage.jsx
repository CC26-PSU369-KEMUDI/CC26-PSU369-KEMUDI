  ...
  useEffect(() => {
    const fetchDashboardHistory = async () => {
      try {
        ...
        //   return sum + Number(item.total_drowsy || 0);
        // }, 0);
        console.log(data[0])
        const totalDrowsy = data[0].frekuensi;

        // =====================================
        // TOTAL MONITORING
        // =====================================
        const [hours, minutes, seconds] = data[0].durasi.split(':')

        const totalMonitoringSeconds = Number(minutes) * 60 + Number(seconds);
